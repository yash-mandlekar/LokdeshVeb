import React from "react";
import "./userRessl.css";

const UserReels = () => {
  return (
    <>
      <div className="user-reels">
        <div className="userReelsPhoto">
            <img src="https://images.unsplash.com/photo-1606227393416-94ea103b1502?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1332&q=80" alt="" />
        </div>
        <div className="userallreels">
        <div className="usermainreels"><img src="https://images.unsplash.com/photo-1674295648958-d7883814585d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1887&q=80" alt="" /></div>
        <div className="usermainreels"><img src="https://images.unsplash.com/photo-1674295648958-d7883814585d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1887&q=80" alt="" /></div>

        <div className="usermainreels"><img src="https://images.unsplash.com/photo-1674295648958-d7883814585d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1887&q=80" alt="" /></div>

        <div className="usermainreels"><img src="https://images.unsplash.com/photo-1674295648958-d7883814585d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1887&q=80" alt="" /></div>

        <div className="usermainreels"><img src="https://images.unsplash.com/photo-1674295648958-d7883814585d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1887&q=80" alt="" /></div>
        <div className="usermainreels"><img src="https://images.unsplash.com/photo-1674295648958-d7883814585d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1887&q=80" alt="" /></div>

        <div className="usermainreels"><img src="https://images.unsplash.com/photo-1674295648958-d7883814585d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1887&q=80" alt="" /></div>

        <div className="usermainreels"><img src="https://images.unsplash.com/photo-1674295648958-d7883814585d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1887&q=80" alt="" /></div>

        <div className="usermainreels"><img src="https://images.unsplash.com/photo-1674295648958-d7883814585d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1887&q=80" alt="" /></div>

        <div className="usermainreels"><img src="https://images.unsplash.com/photo-1674295648958-d7883814585d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1887&q=80" alt="" /></div>

        <div className="usermainreels"><img src="https://images.unsplash.com/photo-1674295648958-d7883814585d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1887&q=80" alt="" /></div>

        <div className="usermainreels"><img src="https://images.unsplash.com/photo-1674295648958-d7883814585d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1887&q=80" alt="" /></div>

        <div className="usermainreels"><img src="https://images.unsplash.com/photo-1674295648958-d7883814585d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1887&q=80" alt="" /></div>

        <div className="usermainreels"><img src="https://images.unsplash.com/photo-1674295648958-d7883814585d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1887&q=80" alt="" /></div>

        <div className="usermainreels"><img src="https://images.unsplash.com/photo-1674295648958-d7883814585d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1887&q=80" alt="" /></div>

        <div className="usermainreels"><img src="https://images.unsplash.com/photo-1674295648958-d7883814585d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1887&q=80" alt="" /></div>


        </div>
      </div>
    </>
  );
};

export default UserReels;
